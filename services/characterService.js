...
exports.searchCharacters = async (name, server = '', dataCenter = '') => {
  ...
  try {
    ...
    $('.entry__link').each((i, element) => {
      ...
      const $characterLang = $element.find('.entry__chara__lang');
...
exports.getCharacterDetails = async (id) => {
  try {
    ...
    $('.character__level__list li').each((i, element) => {
      ...
      let jobName = $element.find('img').attr('data-tooltip');
      if (/[\/()]/.test(jobName)) {
        jobName = jobName.split(/[\/()]/)[0].trim();
      }

      const jobImg = $element.find('img').attr('src');
